...
// Creates Remove selected items button if the list is not empty - doesn't work!
/*
function addRemoveSelectedButton() {
	if (ul.children > 0) {
		let removeSelectedButton = document.createElement("button");
		let selectedButtonContainer = document.getElementById("remove-selected");

		removeSelectedButton.textContent = "Remove selected items";

		selectedButtonContainer.appendChild(removeSelectedButton);
		}
}
addRemoveSelectedButton();
*/
// Select the li elements from the document
